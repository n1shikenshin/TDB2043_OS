...
void input()
{
	...
	scanf("%d", &n);
	printf("Enther the number of resource types: ");
	scanf("%d", &r);
	...
	for (i=0; i<n; i++){
		printf("Process P%d:\n", i+1);
		for (j=0; j<r; j++){
			printf("Resource of type R%d currently allocated: ",j+1);
			scanf("%d", &alloc[i][j]);
		}
	}
	...
	for (j=0; j<r; j++) {
		printf("Avaialble instances of Resource R%d: ", j+1);
		scanf("%d", &avail[j]);
	}
}
...
void detectDeadlock() {
	...
	//Step 4: Detect deadlock processes
	for (i=0; i<n; i++) {
		if (finish[i] == 0) {
			dead[deadCount++] = i;
		}
